...
import pl.kregiel.kuzio.psk.projekt.exception.RezerwacjaException;
...
public class Controller {
    ...
    public Rezerwacja_Response utworz_rezerwacje(
            @PathVariable int sala, @RequestBody Rezerwacja_Request request
    ) {
        try {
            rezerwacja_service.utworz_rezerwacje(request.getId_prowadzacy(), sala, request);
            return new Rezerwacja_Response(true, "Utworzono rezerwacje!");

        }
        catch (RezerwacjaException e) {
            return new Rezerwacja_Response(false, e.getMessage());
        }
        catch (Exception e) {
            return new Rezerwacja_Response(false, "Blad: " + e.getMessage());
        }
    }
    ...
    @DeleteMapping("/{pietro}/sale/{sala}/rezerwacje/{id}")
    public Rezerwacja_Response usun_rezerwacje(@PathVariable int id, @RequestBody Map<String, String> body) {
        try {
            int prowadzacy = Integer.parseInt(body.get("id_prowadzacy"));
            String kod_totp = body.get("kod_totp");
            rezerwacja_service.usun_rezerwacje(prowadzacy, id, kod_totp);
            return new Rezerwacja_Response(true, "Usunieto rezerwacje!");
        }
        catch (RezerwacjaException e) {
            return new Rezerwacja_Response(false, e.getMessage());
        }
