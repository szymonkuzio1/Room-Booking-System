...
public class Controller {
    ...
    public Rezerwacja_Response utworz_rezerwacje(@PathVariable int sala, @RequestBody Rezerwacja_Request request
        ...
        boolean utworzono_rezerwacje = rezerwacja_service.utworz_rezerwacje(request.getId_prowadzacy(),sala, request);
    ...
    public Rezerwacja_Response usun_rezerwacje(@PathVariable int id, @RequestBody Map<String, String> body) throws Exception {

        int prowadzacy = Integer.parseInt(body.get("id_prowadzacy"));
        String kod_totp = body.get("kod_totp");

        boolean usunieto = rezerwacja_service.usun_rezerwacje(prowadzacy, id, kod_totp);
        if (usunieto) {
            return new Rezerwacja_Response(true, "Usunięto rezerwację!");
        }
        else {
            return new Rezerwacja_Response(false, "Nie usunięto rezerwacji!");
        }
    }
}
